...
function BestInPlaceEditor(e, ev) {
  if(ev === undefined) {
    this.evt = 'click';
  }
  this.element = e;
  ...
  this.bindForm();
  this.initNil();
  jQuery(this.activator).bind(this.evt, {editor: this}, this.clickHandler);
}
...
jQuery.fn.best_in_place = function(ev) {
  if (ev === undefined) {
    ev = 'click';
  }
  function setBestInPlace(element, evt) {
    if (ev === undefined) {
      evt = 'click';
    }
    if (!element.data('bestInPlaceEditor')) {
      element.data('bestInPlaceEditor', new BestInPlaceEditor(element, evt));
      return true;
    }
  }

  jQuery(this.context).delegate(this.selector, ev, function () {
    var el = jQuery(this);
    if (setBestInPlace(el, ev))
      el.click();
  });

  this.each(function () {
    setBestInPlace(jQuery(this, ev));
  });
